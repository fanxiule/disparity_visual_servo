...
from .feature_extractor import FeatureExtractor, BaselineExtractor
from .cost_generator import CostGenerator
from .raw_disp_processor import DispProcessor
from .disp_estimator import DispEstimator
from .disp_regressor import DispRegressor
from .disp_refiner import DispRefiner, BaselineRefiner
...
class CRDFusionNet(nn.Module):
    def __init__(self, scale_list, max_disp, img_h, img_w, baseline=False, fusion=True):
        ...
        if baseline:  # use baseline modules
            self.models['extractor'] = BaselineExtractor(scale_list[-1])
            self.models['disp_refine'] = BaselineRefiner(scale_list)
        else:  # use the proposed modules
            self.models['extractor'] = FeatureExtractor(scale_list[-1])
            self.models['disp_refine'] = DispRefiner(scale_list, img_h, img_w)

        # common modules
        self.models['disp_proc'] = DispProcessor(max_disp_at_scale, down_scale)
        self.models['cost_gen'] = CostGenerator(max_disp_at_scale)
        self.models['disp_est'] = DispEstimator()
        self.models['disp_reg'] = DispRegressor(max_disp_at_scale, fusion)
