...
class DatasetConverter:
    def __init__(self, baseline, focal_length, save_path):
        ...
        disp_sub = Subscriber("/camera/depth/image_rect_raw", Image)
        synch = ApproximateTimeSynchronizer(
            [left_sub, right_sub, disp_sub], queue_size=10, slop=0.05)
    ...
    def _callback(self, left, right, disp):
        ...
        raw_disp = self.baseline * self.focal_length / raw_disp
        raw_disp = np.nan_to_num(raw_disp)
        raw_disp[raw_disp >= 192.0] = 0.0
